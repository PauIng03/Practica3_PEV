...
public class PlayerDecetion : MonoBehaviour
{
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        var direction = Quaternion.AngleAxis(FieldOfView / 2, transform.forward) * transform.right;
        Gizmos.DrawRay(transform.position, direction * DetectionRange);

        var direction2 = Quaternion.AngleAxis(FieldOfView / 2, transform.forward) * transform.right;
    ...
    void Update()
    {
        if (IsPlayerClose())
        {
            Debug.Log(IsInFieldOfView());
        }
    }

    private object IsInFieldOfView()
    {
